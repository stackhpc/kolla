...
optional. For further customisation, the block can be overridden via the
template_override mechanism.
...
``ubuntu``, ``ubuntu-security``, and ``ubuntu-cloud-archive``.
...
For RPM-based distros, any existing ``.repo`` file on the image containing
that repository ID is automatically removed before the new one is created.
...
.. code-block:: yaml
   ...
   ubuntu:
     ...
     ubuntu-cloud-archive:
       ...
       suite: "noble-updates/gazpacho"
       url: "https://my-mirror.example.com/ubuntu-cloud-archive"
